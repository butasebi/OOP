...
public:
    ...
    Nod_dublu(const Nod_dublu & alfa);
    void set_ante(Nod_dublu & node);
    void set_ante(Nod* node);
    void set_ante_null();
    //void set_next(Nod_dublu & node);
    Nod* get_ante();
    //Nod* get_next();
    //int get_info();
    ~Nod_dublu();
...
//void Nod_dublu :: set_next(Nod_dublu & node)
//{
    //Nod :: set_next(node);        // apelez fortat functia din clasa nod, altfel o sa o ia pe cea din nod_dublu si o sa fie bucla infinita
    //this -> set_next(node);
//}
...
/*
Nod* Nod_dublu :: get_next()
{
    return this -> Nod :: get_next();
}

int Nod_dublu :: get_info()
{
    return this -> Nod :: get_info();
}
*/
...
public:
    ...
    LDI();
    LDI(Nod_dublu & node);              //constructor de initializare cu un nod dublu
    ...
    Nod_dublu & get_nod(int i) const;
    void LBBS();                    // Lower Bound Binary Search
...
void LDI :: swap_nodes(int a, int b)
{
    ...
    Nod_dublu aux_b(this -> Lista[b]);
    std :: swap(this -> Lista[a], this -> Lista[b]);
    if(a > b)
    {
        int aux = a;
        a = b;
    ...
    if(a + 1 == b)
    {
        if(this -> Lista[a].get_ante() != NULL)
            {
                this -> Lista[a].get_ante() -> set_next(this -> Lista[b]);   // a - 1 -> b
                this -> Lista[b].set_ante(this -> Lista[a].get_ante());      // a - 1 <- b
            }
        if(aux_b.get_next() != NULL)
            {
                static_cast<Nod_dublu*>(aux_b.get_next()) -> set_ante(this -> Lista[a]);  // a <- b + 1  // DownCasting
                this -> Lista[a].set_next(aux_b.get_next());                              // a -> b + 1
            }
        this -> Lista[b].set_next(this -> Lista[a]);                            // b -> a
        this -> Lista[a].set_ante(this -> Lista[b]);                            // b <- a
    }
    else
    {
        if(this -> Lista[a].get_ante() != NULL)
        ...
        if(this -> Lista[a].get_next() != NULL)
        {
            static_cast<Nod_dublu*>(this -> Lista[a].get_next()) -> set_ante(this -> Lista[b]);  // b <- a + 1  // DownCasting
            this -> Lista[b].set_next(this -> Lista[a].get_next());                              // b -> a + 1
        }
        if(aux_b.get_ante() != NULL)
        {
            aux_b.get_ante() -> set_next(this -> Lista[a]);   // b - 1 -> a
            this -> Lista[a].set_ante(aux_b.get_ante());      // b - 1 <- a
        }
        if(aux_b.get_next() != NULL)
        {
            static_cast<Nod_dublu*>(aux_b.get_next()) -> set_ante(this -> Lista[a]);  // a <- b + 1  // DownCasting
